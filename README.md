...
- **グループ化 (ネスト対応)** — 複数オブジェクトをグループ化 / グループ解除できます。グループを含む選択をさらにグループ化するとネストしたグループになり、OOXML でも `p:grpSp` の入れ子として書き出されます。両端が選択内にあるコネクタはグループ化時に自動的にグループへ含まれます (コネクタも `p:cxnSp` としてグループ内に書き出し・読み込み)。グループはまとめて移動・リサイズできます。キャンバスでグループ内の要素をダブルクリックすると 1 階層ずつ選択が降りていき (ネストしたグループは直下のグループ → その中の要素の順)、選択後はその階層のままクリック・ドラッグで兄弟要素の選択や移動ができます。
...
- **線の描画** — ツールバーの「線」から種類 (直線 / カギ線) を選び、キャンバスをドラッグして線を描けます。描画中にオブジェクトへポインタを重ねると接続可能な点 (上下左右) が表示され、端点がオブジェクトの上にあればそのオブジェクトに接続し、なければ自由な点として配置されます (どちらもコネクタとして機能します)。
- **コネクタ** — 2 つのオブジェクトを選択してツールバーまたは右クリックの「コネクタで接続」で、図形・テキストボックス・グラフを結ぶコネクタ (直線 / カギ線、線色・太さ、始点/終点それぞれの矢印の種類 (なし・三角・細三角・線・ひし形・円) とサイズ) を作成できます。矢印は OOXML の `a:headEnd` / `a:tailEnd` (type / w / len) として書き出されます。接続先の移動・リサイズに自動追従し、書き出しは OOXML 準拠の `p:cxnSp` + `a:stCxn`/`a:endCxn` (意味的な接続参照) として出力されるため、**PowerPoint 上で図形を動かしてもコネクタが追従する本物のコネクタ**になります。接続サイトは上下左右の 4 点 (接続サイト番号は rect 系の並びで近似)。コネクタを選択すると始点・終点にハンドルが表示され、**ドラッグで別のオブジェクト / 接続サイトに付け替え**できます (ドラッグ中は候補サイトと最寄り点をハイライト表示)。インスペクタから始点と終点の反転もできます。カギ線は接続サイトの向きに沿って直交ルーティングされ、**繋ぐオブジェクトに触れないようクリアランスを確保**し、位置関係に応じてコーナー数を自動調整します (正対＝2 コーナー、回り込みが必要な配置＝最大 4 コーナー。書き出し時は bentConnector2〜5 の対応プリセットとして出力)。折れ位置は、コネクタ選択時に中央セグメント上へ表示されるハンドルのドラッグ、またはインスペクタの「折れ位置」スライダーで調整できます。回り込むカギ線では始点側・終点側の折れ線にもハンドルが表示され、それぞれドラッグで位置を調整できます。端点ハンドルを何もない場所へドロップすると接続を外して自由な端点に戻せます。
